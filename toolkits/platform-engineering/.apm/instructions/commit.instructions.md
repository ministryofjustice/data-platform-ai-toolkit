...
When creating commits through the GitHub API (e.g. in a cloud agent context with no local `git checkout`), always use the **GraphQL `createCommitOnBranch` mutation** instead of the REST `PUT /repos/{owner}/{repo}/contents/{path}` endpoint. GitHub signs commits made via this mutation server-side, satisfying repository commit signature verification requirements
